...
現時点ではドキュメント、Issue / PRテンプレート、AI向けSkills、Serena設定のみがあります。
アプリ本体、パッケージ定義、起動・ビルド・lint・typecheck・testコマンド、GitHub Actionsワークフローはまだありません。
技術決定後に、実装と一致するセットアップ手順と検証コマンドを追加します。
...
| 現在のStatus・Scope | GitHub Projects（リポジトリのProjectsタブで対象を確認） |
| 採用した重要な技術・設計判断 | `docs/decisions/` のADR（最初の判断時に作成） |
